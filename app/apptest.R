...
server <- shinyServer(function(input, output) {
  ...
  fit.regression <- reactive({
    ...
    if (input$Model == "Log transforming the predictor (time) variable") {
      ...
      if (!inherits(fit.res, "error")) {
        ...
        fit.res <-   (fit.res) # for the residuals
  ...
  output$reg.plot <- renderPlot({         
    ...
    if (input$Plot == "All profiles together") {
     ...
     # names(df) <- c("ID","VISIT","eij","value")
      ...
      pr1 <- ggplot((df_summary1), aes(x = obs, y =yij, color = id)) +
        ...
        scale_x_continuous(breaks = c(unique(df$obs)),
                           labels = 
                             c(unique(df$obs))
        ) +
  ...
  output$view <- renderTable({
     obs= data.frame(obs=make.data()$obs)
    return(obs=obs)
  })
  ...
  output$reg.plot2 <- renderPlot({         
    ...
    obs= data.frame(obs=make.data()$obs)  # took a while to solve this was required!
    
    foo <- make.data()
    
    f <- fit.regression()$fit.summary
    
    new.dat <- data.frame(obs)
    
    new.dat$pred <- predict(f , newdata= new.dat ,level=0)
    
    
    #create design matrix
    Designmat <- model.matrix(eval(eval(f$call$fixed)[-2]),new.dat[-ncol(new.dat)])
    
    #compute standard error for predictions
    
    predvar <- diag(Designmat %*% f$varFix %*% t(Designmat))
    new.dat$SE <- sqrt(predvar)
    new.dat$SE2 <- sqrt(predvar+f$sigma^2)
    ...
    p1 <- 
      ...
      guides(colour=FALSE) +
      geom_ribbon(aes(ymin=pred-2*SE2,ymax=pred+2*SE2),  alpha=0.2,fill="green") +
